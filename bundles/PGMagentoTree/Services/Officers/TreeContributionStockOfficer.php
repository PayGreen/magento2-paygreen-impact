...
/**
 * Class TreeContributionStockOfficer
 * @package PGMagentoTree\Services\Officers
 */
class TreeContributionStockOfficer extends AbstractService
{
     ...
     * @return bool
     */
    public function isValid(ProductEntityInterface $contribution)
    {
        ...
        if (!$testMaxSaleQuantity) {
            $this->log()->warning("Climate Contribution #{$contribution->id()} as max sale quantity greater than 1. (={$stockItem->getMaxSaleQty()})");
        }

        if (!$testManageStock) {
            $this->log()->warning("Climate Contribution #{$contribution->id()} as stock management enabled.");
        }
     ...
     * @inheritDoc
     */
    public function validate(ProductEntityInterface $contribution)
    {
        ...
        $this->log()->info("Successfully deactivate contribution stock management.");
